/**
 * Tagging runs as a job on the server, one request per step, and the Tags
 * tab watches it: progress, requests spent, and a log of every answer and
 * refusal, with a Stop button that also cuts off a request in flight. What
 * was applied before a stop stays applied.
 *
 * It does not wait on a busy model. Retrying a busy Gemini for up to two
 * hours was tried (2026-09-25) and was not worth sitting through; now a
 * refusal ends the job with a message to try again, and the job is saved
 * as it goes so the tab shows what happened even after a reload.
 *
...
import { ModelError, type CallOptions, type ModelEvent } from './ai';
...
export const isActive = (job: TagJob | null) => job?.status === 'running';
...
export function newJob(listId: string, params: JobParams, now = Date.now()): TagJob {
  return {
    ...
    done: 0, total: 0, added: 0, removed: 0, proposed: 0, requests: 0, refusals: 0,
    models: [], failed: [], error: null, events: [], startedAt: new Date(now).toISOString(), finishedAt: null,
  };
}

const plural = (n: number, word: string) => `${n} ${word}${n === 1 ? '' : /(s|ch)$/.test(word) ? 'es' : 's'}`;
...
export async function runJob(job: TagJob, deps: JobDeps, signal: AbortSignal): Promise<TagJob> {
  ...
  /** One step, one request: an answer, or the reason there was none. */
  const once = async <T extends { model: string | null }>(work: (options: CallOptions) => Promise<T>): Promise<T> => {
    const result = await work({ maxBusy: 1, signal, onEvent });
    if (result.model) {
      job.requests++;
  ...
  try {
    if (job.params.kind === 'propose') {
      ...
      save();
      const r = await once((o) => deps.propose(job.params.instructions ?? '', o));
      ...
      for (const keys of cardBatches) {
        ...
        try {
          const r = await once((o) => deps.assign(keys, o));
          job.added += r.added;
      ...
      if (job.params.audit && anyTagged && tags.length && !signal.aborted) {
        ...
        for (const ids of evenBatches(tags, size.tags)) {
          if (signal.aborted) break;
          try {
            const r = await once((o) => deps.audit(ids, o));
            job.added += r.added;
